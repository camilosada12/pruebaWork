...
public class RolUserRepository : BaseModelData<RolUser, RolUserDto>
{
    ...
    {
        var rolUser = await _context.RolUsers
            .Include(ru => ru.Rol)
            .FirstOrDefaultAsync(ru => ru.userid == userId && !ru.active);
    ...
    public async Task AsignarRol(int userId, int rolId)
    {
        ...
        _context.RolUsers.Add(nuevaRelacion); // Agrega la nueva relación al contexto
